...
@router.post("/logos/reference-images-file", response_model=ReferenceUploadResponse)
def upload_reference_images_file(
    db: DbSession,
    user_id: str = Form(...),
    logo_name: str = Form(...),
    files: list[UploadFile] = File(...),
) -> ReferenceUploadResponse:
    cleaned_logo_name = logo_name.strip()
    if not cleaned_logo_name:
        raise HTTPException(status_code=400, detail="logo_name must not be empty")
    if not files:
    ...
    job = create_job(
        db,
        user_id=user_id,
        job_type="reference_upload_file",
        params={
            "logo_name": cleaned_logo_name,
            "filenames": [upload.filename or "uploaded-image" for upload in files],
        },
        logo_id=logo.id,
    )
    ...
    try:
        ...
        for upload in files:
            ...
            normalized_bytes, content_type = image_to_png_bytes(image)
            storage_key = f"reference/{user_id}/{logo.id}/{uuid.uuid4()}.png"
            storage.upload_bytes(storage_key, normalized_bytes, content_type)
            storage_url = storage.build_object_url(storage_key)
            ...
            reference = LogoReferenceImage(
                user_id=user_id,
                logo_id=logo.id,
                source_url=f"upload://{upload.filename}" if upload.filename else None,
                storage_key=storage_key,
                storage_url=storage_url,
                original_filename=upload.filename or "uploaded-image",
                content_type=content_type,
                width=image.width,
                height=image.height,
            )
        ...
        logger.info(
            "Starting Jina embedding request for uploaded references: images=%d user_id=%s logo_id=%s job_id=%s timeout_seconds=%s",
            len(data_urls),
            user_id,
            str(logo.id),
            str(job.id),
        ...
        try:
            ...
            elapsed = time.perf_counter() - embedding_started
            logger.exception(
                "Reference file embedding failed after %.2fs: images=%d user_id=%s logo_id=%s job_id=%s first_filename=%s",
                elapsed,
                len(data_urls),
                user_id,
                str(logo.id),
        ...
        logger.info(
            "Uploaded reference embedding completed in %.2fs: vectors=%d user_id=%s logo_id=%s job_id=%s",
            time.perf_counter() - embedding_started,
            len(vectors),
            user_id,
            str(logo.id),
        ...
        logger.exception(
            "Reference file upload job failed: user_id=%s logo_name=%s logo_id=%s job_id=%s error=%s",
            user_id,
            cleaned_logo_name,
            str(logo.id),
            str(job.id),
...
@router.post("/products/classify-logo-file", response_model=ClassifyLogoResponse)
def classify_logo_file(
    user_id: str = Form(...),
    file: UploadFile = File(...),
    db: Session = Depends(get_db),
) -> ClassifyLogoResponse:
    """Classify logo by uploading a file directly."""
    pipeline = get_logo_pipeline_service()
    storage = get_object_storage_service()
    ...
    try:
        ...
        logger.exception("File upload classification failed: %s", exc)
        mark_job_failed(db, job, str(exc))
        raise HTTPException(status_code=500, detail=str(exc)) from exc
